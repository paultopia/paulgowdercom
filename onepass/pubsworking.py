...
with open('data.json') as cites_file:
  citejson = json.load(cites_file)

# this is incredibly hackish, but it will take forever to clean up and put in a proper template.  

citejson['Articles'] = sorted(citejson['prarts'] + citejson['lrarts'], key = lambda x: int(x['year']), reverse = True)
...
chapterblock = '\n'.join([chapLayout(x) for x in citejson['chapters']])
modalsblock = '\n'.join(popups)
